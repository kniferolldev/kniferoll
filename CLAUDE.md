...
The eval test cases contain copyrighted recipe data and live in a separate private repo (`kniferolldev/kniferoll-evals`). Clone it wherever you like and point the eval command at it:

```bash
bun run kr eval <your-evals-path>                           # compare to baseline
bun run kr eval <your-evals-path> --regenerate --save       # re-run importer + save
bun run kr eval <your-evals-path> --diff                    # show per-field breakdown
```

Commits to the eval data go directly in that repo — no special dance required.

### Test case structure

Each test case is a directory containing:
- `input.txt` (text input) or `image*.jpg` (images) — always save input locally, never fetch at eval time
- `golden.md` — human-edited expected output (quality test cases)
- `actual.md` + `extracted.json` — generated by `--regenerate`, committed alongside golden

For **rejection test cases** (non-recipe input that should be refused):
- `input.txt` — the non-recipe content
- `expected_error.txt` — presence of this file marks the case as a rejection test (content doesn't matter)

After adding cases, run with `--regenerate --save` to generate `actual.md` files, then commit.
